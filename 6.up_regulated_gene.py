...
def seq_extract(seq,input):
    ...
    outputfile = "differential_gene_sequence_up.fasta"
    data = pd.read_csv(os.path.abspath(input) + "/" + "differential_genes.csv")
    df1= data.set_index(['GeneID'])['change'].to_dict()
    df2 = {k: v for k, v in df1.items() if v =='Up'}
    #df2 = {k: v for k, v in df1.items() if v == 'Stable'}
    for key in df2.keys():
        with open(input +'/differential_genes_id_up.txt', 'a') as f:
            f.write(key)
            f.write('\r\n')
    path = os.path.abspath(input + "/" + "differential_genes_id_up.txt")
    cmd2 = "seqkit grep -f "+ path +' ' +seq + " -o " + input + "/"+ outputfile
    run_command(cmd2)
    path2 = os.path.abspath(input + "/" + "differential_gene_sequence_up.fasta")
    path3 = os.path.abspath(input + "/" + "differential_gene_sequence_up.pep")
    cmd3 = "seqkit translate " + path2 + " > " + path3
    run_command(cmd3)
    print(">>>>>>>>>> Differential up-regulated gene sequences extraction finished...................")
####################################################################################################################
def main():
    input,seq = Options()
